DROP TRIGGER IF EXISTS trg_insert_matching_review;      -- 일대일 매칭에서 작성된 후기로 멤버의 평점을 계산하고 등급을 조정

DELIMITER //

CREATE TRIGGER trg_insert_matching_review
AFTER INSERT ON matching_review
FOR EACH ROW
BEGIN
    ...
    SELECT 
     ...
     WHERE id = (
	               SELECT 
	                ...
	                WHERE member_matching.id = NEW.id
	             );
    SELECT 
	        weight INTO weight_
      FROM rank
